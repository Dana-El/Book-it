                ...
                borrowRef.addListenerForSingleValueEvent(object : ValueEventListener {
                    ...
                    override fun onDataChange(snapshot: DataSnapshot) {
                        ...
                        children.forEach { it ->
                            ...
                            val bookname :TextView= vv.findViewById(R.id.given_bookname)
                            val groupname :TextView= vv.findViewById(R.id.given_authorname)
                            val ownername :TextView= vv.findViewById(R.id.given_person)
                            var name = it
                            if(it.child("owner").value == firebaseAuth.getCurrentUser()?.getUid()){
                                        ...
                                        userRef.addListenerForSingleValueEvent(object :
                                            ...
                                            override fun onDataChange(snapshot: DataSnapshot) {
                                                ...
                                                children.forEach {it2->
                                                    ...
                                                    println(test)
                                                    println(it.child("borrower").value)
                                                    println(" ")
                                                    if (it2.child("uid").value.toString() ==it.child("borrower").value.toString()){
